...
<a href="<?php echo $helper->url("Admin","index"); ?>"><button type="button" class="btn btn-primary btn-lg">Usuarios</button></a>
    ...
    <a href="<?php echo $helper->url("Muro","cerrarSesion"); ?>"><button type="button" class="btn btn-dark btn-lg">Salir</button></a>
      ...
      <div class="card text-center">
        ...
        <div class="overflow-auto" style="height:42rem;">
          <form action="<?php  echo $helper->url("Admin","administrarMods");?>" method="post">
                    <table class="table">
        ...
        foreach($allUsers as $user){$estado; if($user->ESTADO==1){$estado='Activo';}else {$estado='Desactivado';}
          echo '
                        <tr>
                            <th scope="row">'.$user->ID_USUARIO_MOD.'</th>
                            <td>'.$user->USERNAME.'</td>
                            <td>'.$user->MAIL.'</td>
                            <td>'.$user->USUARIO_ALTA.'</td>
                            <td>'.$user->FECHA_ALTA.'</td>
                            <td>'.$user->FECHA_ULT_MOD.'</td>
                            <td>'.$estado.'</td>  
                            <td><input type="checkbox" name="ids[]" value="'.$user->ID_USUARIO_MOD .'"></td>                             
                        </tr>
                  ';
          }
        ?>
     ...
     <input type="submit" name="desactivar" class="btn btn-primary" value="Desactivar Cuenta">
     <input type="submit" class="btn btn-danger" name="eliminarmod" value="Eliminar Moderador">
     </div>
     </form>
